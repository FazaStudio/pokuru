import { useTonConnection } from '../../hooks/ton/useTonConnection';
...
export const TonWalletButton: React.FC<TonWalletButtonProps> = ({
  ...
  const { connect, disconnect } = useTonConnection();
  ...
  return (
    ...
    >
      <img src="/icons/wallets/ton.svg" alt="TON Wallet" className="w-8 h-8" />
      <div className="flex flex-col items-start">
        <span className="text-white text-sm font-medium">
          {isConnected ? 'Disconnect' : 'Connect TON Wallet'}
        </span>
        {isConnected && address && (
